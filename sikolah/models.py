...
class Siswa(db.Model):
    ...
    nilai = db.relationship('Nilai', backref=db.backref(
        'data_nilai_siswa', lazy=True))
...
class Pelajaran(db.Model):
    ...
    nilai = db.relationship('Nilai', backref=db.backref(
        'data_pelajaran_siswa', lazy=True))
...
class Nilai(db.Model):
    ...
    id_pelajar = db.Column(db.Integer, db.ForeignKey(
        "pelajaran.id"), nullable=False)

    siswa = db.relationship('Siswa', backref=db.backref(
        'data_siswa', lazy=True))
...
#         "Pelajaran", backref="data_nilai", lazy=True, secondary='nilai')
#     user = db.relationship("User", backref='data_user',
...
#     id = db.Column(db.Integer, primary_key=True, nullable=False)
#     nama_pelajaran = db.Column(db.String(50), nullable=False)
#     siswa = db.relationship("Siswa", lazy=True, secondary='nilai')
...
#         'Siswa', backref=db.backref('datanilai', lazy='dynamic'))
#     pelajaran = db.relationship(
#         'Pelajaran', backref=db.backref('datanilai', lazy='dynamic'))
